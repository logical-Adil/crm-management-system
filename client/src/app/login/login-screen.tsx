...
export function LoginScreen() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-background">
      ...
      <div className="pointer-events-none absolute -left-32 top-1/4 h-72 w-72 rounded-full bg-primary/[0.06] blur-3xl" aria-hidden />
      <div className="pointer-events-none absolute -right-24 bottom-0 h-80 w-80 rounded-full bg-slate-400/[0.08] blur-3xl" aria-hidden />
      <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_bottom,transparent,rgba(250,250,250,0.8))]" aria-hidden />
        ...
        <div className="w-full max-w-[420px]">
            ...
            <div className="rounded-t-[14px] bg-surface px-8 pb-0 pt-8">
              <div className="flex flex-col items-center gap-4 text-center">
                ...
                <div>
                  ...
                  <p className="mt-1.5 text-body leading-relaxed text-muted">Enter your work email and password to continue</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-b-2xl border border-t-0 border-border bg-surface px-8 pb-8 pt-6 shadow-[0_24px_48px_-12px_rgba(15,23,42,0.12)] ring-1 ring-slate-900/[0.04]">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
              {rootError && (
                <div className="rounded-control border border-danger/25 bg-red-50 px-3 py-2.5 text-center text-small text-red-800" role="alert">
                  {rootError}
                </div>
              )}

              <div className="space-y-1.5">
                <label htmlFor="login-email" className="block text-small font-medium text-foreground">
                  Email
                </label>
                <input
                  id="login-email"
              ...
              <div className="space-y-1.5">
                <label htmlFor="login-password" className="block text-small font-medium text-foreground">
                  Password
                </label>
                <input
                  id="login-password"
              ...
              <button type="submit" disabled={isSubmitting} className={`${loginSubmitButton} mt-2`.trim()}>
                <span className="relative z-10 flex items-center justify-center gap-2">
                  {isSubmitting && <span className="h-4 w-4 animate-spin rounded-full border-2 border-primary-foreground/30 border-t-primary-foreground" aria-hidden />}
            ...
            <div className="mt-6 border-t border-border pt-5 text-center">
              <p className="text-small text-muted">
                Seed example:
                <br />
                <code className="rounded-md bg-slate-100 px-2 py-0.5 font-mono text-xs text-slate-700">email: admin1@acme.demo</code>
                <br />
                <code className="rounded-md bg-slate-100 px-2 py-0.5 font-mono text-xs text-slate-700">password: Adil030400</code>
              </p>
            </div>
          </div>
        </div>
